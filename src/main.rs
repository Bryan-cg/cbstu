...
fn benchmark() {
    ...
    for path in paths {
        ...
        if path.ends_with(".json") {
            ...
            let list = vec![end_berman, end_ee];
            ...
            times.push((graph.nodes().len(), graph.edges().len(), end_berman, end_ee, bottleneck4, fastest_name));
    ...
    println!("Results:");
    println!("Nodes \t Edges \t Berman[ms] \t EE \t Bottleneck \t Fastest");
    for (v, e, berman, end_EE, bottleneck, fastest) in &times {
        println!("{} \t {} \t {:.3} \t {} \t {} \t {}", v, e, berman, end_EE, -bottleneck, fastest);
    }
}
