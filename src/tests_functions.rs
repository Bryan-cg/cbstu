...
mod tests {
    ...
    //use std::{println as info, println as warn, println as trace, println as debug}; // Workaround to use println! for logs.
    ...
    fn test_easy_instance() {
        ...
        let mut duplicated_graph = Util::duplicate_edges_mut(&graph_mut);
    ...
    fn test_difficult_instance2() {
        ...
        let (_, _, bottleneck_3, _) = EdgeElimination::run(&mut duplicated_graph, 1000.0);
        duplicated_graph = Util::duplicate_edges_mut(&graph_mut);
        ...
        let (_, _, bottleneck_small3, _) = EdgeElimination::run(&mut duplicated_graph, 100.0);
        duplicated_graph = Util::duplicate_edges_mut(&graph_mut);
        ...
        let (_, _, bottleneck_mid3, _) = EdgeElimination::run(&mut duplicated_graph, 700.0);
        duplicated_graph = Util::duplicate_edges_mut(&graph_mut);
